  ...
  -- customize alpha options
  {
    ...
    config = function()
      ...
      if vim.fn.has "linux" == 1 or vim.fn.has "mac" == 1 then
        local handle = io.popen 'ls -l $HOME"/.local/share/nvim/lazy" |  wc -l | tr -d "\n" '
        if handle ~= nil then
      ...
      local function button(sc, txt, keybind)
        ...
        if keybind then btn_opts.keymap = { "n", sc_, keybind, { noremap = true, silent = true } } end
      ...
      local buttons = {
        type = "group",
        val = {
          button("f f", "  Find files", "<Cmd>lua require('telescope.builtin').find_files()<CR>"),
          button("f o", "󰥔  Find old files", "<Cmd>lua require('telescope.builtin').oldfiles()<CR>"),
          button("S f", "󰶾  Load session", "<Cmd>lua require('resession').load()<CR>"),
          button("S l", "󰻢  Load last session", "<Cmd>lua require('resession').load('Last Session')<CR>"),
          button(
            "g g",
            "  Git status",
            "<Cmd>lua require('astrocore').toggle_term_cmd('lazygit " .. flags .. "')<CR>"
          ),
          button("n", "󰈚  New file", "<Cmd>enew<CR>"),
          button("q", "  Quit window", "<Cmd>confirm q<CR>"),
        },
        opts = {
          spacing = 1,
        },
